Template.matryoshka__fields.helpers({
	...
	selectValueIsSelected: function ( context, currentValue ) {
		if (!this || !context || !currentValue || !context.name || !currentValue[ context.name ])
			return null;
		if (this.name == currentValue[ context.name ])
			return 'selected';
	}
});

Template.matryoshka__fields.events({
	'click .go-to-linked-nestable-link': function () {
		Router.go('/matryoshka/edit/' + Template.parentData(1).nestableToLinkId );
	},
	'click .change-linked-nestable': function () {
		var parentContext = Template.parentData(3);
		var linkedPartContext = Template.parentData(1);
		Matryoshka.partOverlay.show( parentContext, 'updatePart', linkedPartContext.matryoshkaId );
	}
});