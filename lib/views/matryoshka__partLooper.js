...
Template.matryoshka__partLooper.helpers({
	partIsFocusedCss: function () {
		if ( Session.get('matryoshka__focused-nestable') === this.matryoshkaId )
			return 'matryoshka__nestable__container__col--focus';
	},
	...
	fields: function () {
		...
		// Let's create a copy of the fields to return to avoid inheritance of previously stored data
		var fieldsToReturn = [].concat( nestableDefinedInCode.fields );
...
var handleTextInputChange = function ( e, context, parentContext ) {
	...
	if (context.onAfterAction)
		if (typeof context.onAfterAction === 'function')
			value = context.onAfterAction( value, parentContext );
...
Template.matryoshka__partLooper.events({
		'click .matryoshka__nestable__container__col--left': function ( e ) {
			...
			checkIfEventWasJustTriggered = false;
		...
		'change .matryoshka-save-on-select': function ( e ) {
				...
				if (nestableDefinedInCode && nestableDefinedInCode.selectableData) {
						if (nestableDefinedInCode.selectableData.type === 'collection') {
								...
								Matryoshka.currentNestable.update( matryoshkaId, 'put', 'collection-id', collectionId );
						}
				}
		...
		'click .duplicate-part': function (e) {
				...
				Matryoshka.currentNestable.update( matryoshkaId, 'put', key, objectToDuplicate, this.matryoshkaId );
		...
		'click .delete-part': function (e) {
				...
				Matryoshka.currentNestable.update( this.matryoshkaId, 'deletePart' );
		...
		'click .move-part': function (e) {
				...
				Matryoshka.currentNestable.update( parentMatryoshkaId, 'move', key, moveWhere, this.matryoshkaId );

		},
		'click .show-part-list': function (e) {

			Matryoshka.partOverlay.show( this );
