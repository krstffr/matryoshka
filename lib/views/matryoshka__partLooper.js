Template.matryoshka__partLooper.helpers({
	nestables: function() {
		...
		var nestableDefinedInCode = _(Matryoshka.nestables[this.type]).findWhere({ nestableName: this.nestableName });
	...
	nestableGetCssClasses: function ( context ) {
		...
		var cssToReturn = ' ';
		if (context) {
			var parentNestable = Matryoshka.nestablePart.getNestablePartData( context.type, context.nestableName );
			if (parentNestable && parentNestable.hideChildren)
				cssToReturn += 'matryoshka__nestable__container--hidden ';
		}
