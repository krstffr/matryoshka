Template.matryoshka__partLooper.helpers({
    ...
    setActiveSelectValue: function( id, context, name ) {
        ...
        // HERE WE USED TO SET THE CSS CLASS, HOWEVER THIS IS NOT VERY NICE
        // INSTEAD WE SHOULD PROBABLY STORE THE CSS CLASS(ES) SOMEWHERE INSTEAD SO
        // THEY WON'T BE RESET EVERY TIME THE SESSION UPDATES

        if (!this.cssOutputValueAsClass) return ;
        ...
        Meteor.setTimeout(function () {
            ...
            var elToAddCssTo = $('.matryoshka__nestable__container__col--'+context.matryoshkaId).closest('.matryoshka__nestable__container');
            elToAddCssTo.addClass('matryoshkaCssClass--'+that.name+context[ that.name ]);

        }, 1);

    },
    maybeSetCSS: function( context ) {
