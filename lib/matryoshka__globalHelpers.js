...
Handlebars.registerHelper('matryoshka__getFieldValue', function( context ) {
  ...
  if (this.beforeAction) {
    ...
    var updatedSession = Matryoshka.nestables.modifyNestableBasedOnId( Session.get('matryoshkaCurrentNestable'), context.matryoshkaId, 'put', this.name, toReturn );
...
var bodyOverflowInit = false;

matryoshka__focusOnPagePart = function ( idOfPart ) {

  if (!bodyOverflowInit)
    bodyOverflowInit = $('body').css('overflow');
  ...
  if (currContainer.hasClass( cssClassOfFocus )) {
    $('body').css({ overflow: 'hidden' });
  }
  else {
    $('body').css({ overflow: bodyOverflowInit });
  }
