Router.map(function () {
	...
	this.route('matroyshka', {
		...
		before: function () {
			if (this.params.action === 'edit' && this.params._id) {
				var nestableToEdit = MatroyshkaNestables.findOne({ _id: this.params._id });
				Session.set('matroyshkaCurrentNestable', nestableToEdit );
				// Hack to make sure it does not create an infinite loop (or something)
				// Meteor.setTimeout(Matroyshka.updateFieldsInTemplate, 100);
			}
