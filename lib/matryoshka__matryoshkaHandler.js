MatryoshkaHandler = function () {
  ...
  that.userDefinedFields.add = function ( fieldOptions ) {
    ...
    // check() does not work on server for some reason, quick fix!
    if (Meteor.isClient) {

      // Check passed options
      check( fieldOptions.name, String );
      check( fieldOptions.templateFileName, String );

      if (fieldOptions.initMethod)
        check( fieldOptions.initMethod, Function );

      if (fieldOptions.saveMethod) {
        check( fieldOptions.saveMethod, Function );
        // Make sure the saveMethod return an object
        check( fieldOptions.saveMethod({ test: 'this is a test-object' }), Object );
      }
