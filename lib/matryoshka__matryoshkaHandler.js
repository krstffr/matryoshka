MatryoshkaHandler = function () {
  ...
  that.users.requireLogin = function( require, matryoskhaOnly ) {
    ...
    if (typeof require !== 'boolean')
      throw new Error('Matryoshka.users.requireLogin( require ): "require" must be a boolean');
  ...
  that.users.userIsPermitted = function ( userId ) {
    ...
    if (Meteor.isServer && !userId) return false;
  ...
  that.users.deleteUser = function ( userId, username, cb ) {
    ...
    if (typeof userId !== 'string')
      throw new Error('Matryoshka.users.deleteUser( userId, username, cb ): "userId" should be an string, not: ' + typeof userId);

    if (username && typeof username !== 'string')
      throw new Error('Matryoshka.users.deleteUser( userId, username, cb ): "username" should be an string, not: ' + typeof username);

    if (cb && typeof cb !== 'function')
      throw new Error('Matryoshka.users.deleteUser( userId, username, cb ): "cb" must be a function!');

    // Log out current user (if it's the user which gets removed)
    ...
    Meteor.call('matryoshka/users/delete', userId, function (err, res) {
      if (cb)
        ...
        return that.messages.add({ message: 'User "'+username+'" deleted!', type: 'success' });
      }
    });
  ...
  that.users.createUser = function ( newUser, autoLogin, cb ) {
    ...
    if (typeof newUser !== 'object')
      throw new Error('Matryoshka.users.createUser( newUser, autoLogin, cb ): "newUser" should be an object, not: ' + typeof newUser);

    if (!newUser.username || !newUser.password)
      throw new Error('Matryoshka.users.createUser( newUser, autoLogin, cb ): "newUser" must contain username and password');

    if (cb && typeof cb !== 'function')
      throw new Error('Matryoshka.users.createUser( newUser, autoLogin, cb ): "cb" must be a function!');

    if (typeof autoLogin !== 'boolean')
      throw new Error('Matryoshka.users.createUser( newUser, autoLogin, cb ): "autoLogin" must be a boolean!');
    ...
    var userCreatedCallback = function (err) {
      ...
      if (err)
        return that.messages.add({ message: err.reason, type: 'error' });
      return that.messages.add({ message: 'User "'+newUser.username+'" created!', type: 'success' });
    };
  ...
  that.users.changeUserLevel = function ( userId, newLevel ) {
    ...
    if (!userId || typeof userId !== 'string')
      throw new Error('Matryoshka.users.changeUserLevel = function ( userId, newLevel ): "userId" should be an string, not: ' + typeof newUser);

    if (!newLevel || typeof newLevel !== 'string')
      throw new Error('Matryoshka.users.changeUserLevel = function ( userId, newLevel ): "newLevel" should be an string, not: ' + typeof newUser);
