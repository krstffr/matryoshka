MatryoshkaHandler = function () {
  ...
  that.nestablePart.save = function( doc, goLiveBool ) {
    ...
    Meteor.call('matryoshka/nestable/save', doc, goLiveBool, nestableDefinedInCode, function (error, result) {
      ...
      // Make sure the session has actually been reset with the timeOut…
  ...
  that.messages.resetTime = 5000;

  that.messages.removeMessage = function ( msg ) {
    var currentMessages = that.messages.get();
    ...
    that.messages.set( messagesWithoutMatch );
  };
  ...
  that.messages.add = function ( msg ) {
    ...
    that.messages.set( currentMessages );

    // For auto hiding of messages
    Meteor.setTimeout( function () {
      that.messages.removeMessage( msg );
    }, that.messages.resetTime);

  };

  that.messages.get = function ( msg ) {
    return Session.get('matryoshkaMessages') || [];
  };

  that.messages.set = function ( messages ) {
