MatryoshkaHandler = function () {
  ...
  that.nestables.add = function( passedNestable ) {
    ...
    var prohibitedFields = _( _.map(passedNestable.fields, function(field){
      if ( _.indexOf( that.allowedFieldTypes, field.type ) < 0 )
        return field;
    })).compact();
    ...
    passedNestable.fields = passedNestable.fields || [];

    // Nestables should be nestableCreateable by default
    passedNestable.nestableCreateable = passedNestable.nestableCreateable || true;
  ...
  that.nestablePartModifiers.deletePart = function () {
    return false;
  };
  ...
  that.init = function () {
    ...
    if(Meteor.isClient) {
      Meteor.startup(function () {
        ...
        Msgs.hideButton.setButtonHtml('<i class="fa fa-times"></i>');
      });
    }
