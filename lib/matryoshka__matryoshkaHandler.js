MatryoshkaHandler = function () {
  ...
  that.users.userIsPermitted = function ( userId ) {
    ...
    if (Meteor.isClient) {
      if (that.users.allowAllUsers) {
        if (!Meteor.user().matryoshkaLevel) return false;
        if (Meteor.user().matryoshkaLevel === 'unauth') return false;
      }
  ...
  // Create a new user
  that.users.createUser = function ( newUser ) {
    
    if (typeof newUser !== 'object')
      throw new Error('Matryoshka.users.createUser( newUser): "newUser" should be an object, not: ' + typeof newUser);
    
    Meteor.call('matryoshka/users/createUser', newUser );
  ...
  that.users.getAuthorizedUsers = function () {
    ...
    var users = Meteor.users.find({ matryoshkaLevel: { $exists: true } }).fetch();
    return _.filter(users, function(user) { return user.matryoshkaLevel !== 'unauth'; });
  };

  that.users.getNonAuthorizedUsers = function () {
    if (that.users.allowAllUsers)
      return false;
    return Meteor.users.find({ $or: [{ matryoshkaLevel: { $exists: false } }, { matryoshkaLevel: 'unauth' }] });
  };
