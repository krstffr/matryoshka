MatryoshkaHandler = function () {
  ...
  // Delete a user
  that.users.deleteUser = function ( userId, cb ) {

    if (typeof userId !== 'string')
      throw new Error('Matryoshka.users.deleteUser( userId, cb ): "userId" should be an string, not: ' + typeof userId);

    if (cb && typeof cb !== 'function')
      throw new Error('Matryoshka.users.deleteUser( userId, cb ): "cb" must be a function!');
    ...
    Meteor.call('matryoshka/users/delete', userId, function (err, res) {
      if (cb)
        ...
        return that.messages.add({ message: 'User deleted!', type: 'success' });
      }
    });
  ...
  that.users.createUser = function ( newUser, autoLogin, cb ) {
    ...
    if (!autoLogin) {
      Meteor.call('matryoshka/users/create', newUser, function (err, res) {
        if (cb)
          return cb(err, res);
        else {
          if (err)
            return that.messages.add({ message: err.reason, type: 'error' });
          return that.messages.add({ message: 'User created!', type: 'success' });
        }
      });
    } else {
      Accounts.createUser( newUser, function (err, res) {
        if (cb)
          cb(err, res);
        else {
          if (err)
  ...
  that.users.getAuthorizedUsers = function () {
    ...
    var users = Meteor.users.find({ _id: { $ne: Meteor.userId() },  'profile.matryoshkaLevel': { $exists: true } }).fetch();
  ...
  that.users.getNonAuthorizedUsers = function () {
    if (that.users.allowAllUsers)
      return false;
    var users = Meteor.users.find({ _id: { $ne: Meteor.userId() }, $or: [{ 'profile.matryoshkaLevel': { $exists: false } }, { 'profile.matryoshkaLevel': 'unauth' }] }).fetch();
    return that.filter.filterCollection(users, ['username']);
  };
